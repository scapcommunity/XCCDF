...
organization-specific tooling. For example, XCCDF maintains profiles via the Profile element (see Section 6.5), in which all Rules (see Section 6.6.4) that belong to a Profile are referenced. An organization may, however, maintain profile information as part of the rule (thus allowing an author to see with a single glance at the rule, to which profiles the rule belongs). This can be achieved by (1) adding organization-specific metadata regarding profiles and (2) a pre-processing step using organization-specific tooling, by which the XCCDF to be used/published is generated.
