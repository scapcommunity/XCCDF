...
Elements that support the _\<xccdf:metadata\>_ element MAY use any desired metadata format; for _\<xccdf:Benchmark\>_, this is in addition to the Dublin Core recommendations already mentioned. Any XML metadata structures, including ad hoc structures, may be included in an_\<xccdf:metadata\>_element. Because any structures can appear in this field, authors should tag metadata withthe _xmlns_prefix [XMLNAME] and, if available, the_@xsi:schemaLocation_ attribute in order to identify the metadata structure utilized. Whenever possible, metadata should comply with existing commercial or government metadata specifications to allow benchmarks to be discovered and indexed, but organization-specific use-cases for metadata are possible, as well.

In particular, metadata enables the extension of XCCDF with organization-specific features that are not part of the XCCDF standard by preprocessing XCCDF documents based on organization-specific meta-data definitions and
organization-specific tooling. For example, XCCDF maintains profiles via the Profile element (see Section 6.5), in which all Rules (see Section 6.6.4) that belong to a Profile are referenced. An organization may, however, maintain profile information as part of the rule (thus allowing an author to see with a single glance at the rule, to which the profiles the rule belongs). This can be achieved by (1) adding organization-specific metadata regarding profiles and (2) a pre-processing step using organization-specific tooling, by which the XCCDF to be used/published is generated.

Thus, Metadata is a powerful feature for authors, playing an important role in the extensibility of XCCDF. However, XCCDF puts some limits on the use of metadata:
